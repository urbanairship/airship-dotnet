...
public partial class MessageViewHandler : ViewHandler<IMessageView, WKWebView>
{
    ...
    public void LoadUser(string messageId, Action<bool> result)
    {
        UAirship.MessageCenter.Inbox.GetUserWithCompletionHandler(currentUser =>
        {
            UAirship.MessageCenter.Inbox.MessageForID(messageId, currentMessage =>
            {
                ...
                // TODO: UANativeBridge and UAMessageCenterNativeBridgeExtension not exposed in SDK 19 ObjectiveC bindings
                // This functionality is critical for displaying message content
                // TEMPORARY WORKAROUND - See: /Context/SDK19-TEMPORARY-WORKAROUNDS.md section 1
                /*
                nativeBridge = new()
                {
        ...
        else
        {
            UAirship.MessageCenter.Inbox.RefreshMessagesWithCompletionHandler(refresh =>
            {
                if (refresh == true)
                {
                    ...
                    {
                        message = newMessage;
                        // TODO: UAMessageCenterMessage.IsExpired not exposed in SDK 19 ObjectiveC bindings
                        // TEMPORARY WORKAROUND: Assume message is not expired
                        // See: /Context/SDK19-TEMPORARY-WORKAROUNDS.md section 9
                        if (message != null)
                        {
                            LoadMessageBody(message, result);
                        }
    ...
    protected void LoadMessageBody(UAMessageCenterMessage message, Action<bool> result)
    {
        ...
        // TODO: UAUtils.AuthHeaderString not exposed in SDK 19 ObjectiveC bindings
        // TEMPORARY WORKAROUND: Create basic auth header manually
        // See: /Context/SDK19-TEMPORARY-WORKAROUNDS.md section 8
        var credentials = $"{user.Username}:{user.Password}";
        var credentialsData = NSData.FromString(credentials, NSStringEncoding.UTF8);
        var auth = $"Basic {credentialsData.GetBase64EncodedString(NSDataBase64EncodingOptions.None)}";
